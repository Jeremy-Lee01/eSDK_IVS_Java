            ...
            if (!cookie.isExpired(now))
            {
                if (cookieSpec.match(cookie, cookieOrigin))
                {
                    if (this.log.isDebugEnabled())
                    {
                        ...
                        String logMessage = cookie.toString();
                        if ("SessionID".equals(cookie.getName()))
                        {
                            String exp = cookie.getValue();
                            if (null != exp)
                            {
                                StringBuffer rep = new StringBuffer();
                                int length = exp.length() / 2;
                                for (int i = 0; i < length; i++)
                                {
                                    rep.append("*");
                                }
                                rep.append(exp.substring(length));
                                logMessage = logMessage.replace(exp, rep);
                            }
                        }
                        
                        this.log.debug("Cookie " + logMessage + " match " + cookieOrigin);
                    }
